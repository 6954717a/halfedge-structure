...
class PolyMesh
{
    ...
    public:
        ...
        std::vector<MyHalfedge *> vertAdjacentHalfEdge(MyVert * vert) const; //...垂直半边
        std::vector<MyVert *> vertAdjacentVert(MyVert * vert) const; //..垂直点
        std::vector<MyPolyFace *> edgeAdjacentPolygon(MyEdge * edge) const; //某个边对应的相邻多边形
        MyPolyFace * edgeAdjacentAnotherPolygon(MyEdge * edge, MyPolyFace * face) const; //半边的另一个相邻多边形
        bool isBoundary(MyVert * vert) const; //边界判断
        ...
        MyEdge * addEdge(MyVert * v_begin, MyVert * v_end); //加边
